...
function DreamNodePanel({
  ...
  const charSlugVal = charSlug(character)
  const spriteStates: string[] = Object.keys((manifest as any)?.characters?.[charSlugVal]?.sprites ?? {})
  const spriteEntry = (manifest as any)?.characters?.[charSlugVal]?.sprites?.[characterSprite]
  const spriteVersions: string[] = spriteEntry?.versions?.map((v: any) => v.url) ?? []
  const activeSpriteUrl = spriteVersions.length > 0
    ? `${API}/api/projects/${projectId}/assets/${spriteVersions[spriteVersions.length - 1]}`
    : null
  ...
  async function generateBackground() {
    ...
    setBgGenProgress('Submitting…')
    try {
      const res = await fetch(`${API}/api/projects/${projectId}/dream-nodes/${node.id}/generate-background`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: bgPrompt }),
      })
  ...
  return (
    <div>
      ...
      <label style={labelStyle}>Character</label>
      <select style={{ ...inputStyle, cursor: 'pointer' }} value={character} onChange={e => { setCharacter(e.target.value); setCharacterSprite('') }}>
        <option value="">— none —</option>
        ...
        <>
          ...
          {spriteStates.length > 0 ? (
            <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
              ...
              >
                <option value="">— pick state —</option>
                {spriteStates.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              {activeSpriteUrl && (
                <img
                  key={activeSpriteUrl}
            ...
            <div style={{ fontSize: 10, color: '#6b7280', padding: '4px 0' }}>No sprites generated yet.</div>
          )}
        </>
      )}
      ...
      <div style={{ background: '#0f172a', border: '1px solid #374151', borderRadius: 6, padding: 8, marginBottom: 8 }}>
        ...
        <textarea
          ...
          value={bgPrompt}
          onChange={e => setBgPrompt(e.target.value)}
          placeholder="A dreamy underwater cave with glowing bioluminescent plants…"
          disabled={generatingBg}
        />
        <button
          onClick={generateBackground}
          disabled={generatingBg || !bgPrompt.trim()}
          style={{
            width: '100%', background: generatingBg ? '#374151' : '#0f766e', color: 'white', border: 'none',
            borderRadius: 4, padding: '5px 0', fontSize: 11, cursor: (generatingBg || !bgPrompt.trim()) ? 'not-allowed' : 'pointer',
          }}
        >
          {generatingBg ? '⏳ Generating…' : 'Generate Background'}
